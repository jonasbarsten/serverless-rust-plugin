...
class ServerlessRustPlugin {
  constructor(serverless, options, { log }) {
    ...
    this.commands = {
      "rust:start": {
        usage:
          "Start the docker container processes according to configurations and options at once",
        lifecycleEvents: ["start"],
        options: {
          function: {
            usage:
              "The name of the function to start the docker container. If not given, all the rust function starts",
            shortcut: "f",
            type: "multiple",
          },
        },
      ...
      "rust:invoke": {
        usage:
          "Invoke lambda function locally according to configurations and options",
        lifecycleEvents: ["execute"],
        options: {
          function: {
            usage:
              "The name of the function in your service that you want to invoke locally. Required.",
            shortcut: "f",
            type: "string",
            required: true,
          },
          path: {
            usage:
              "The path to a JSON file holding input data to be passed to the invoked function as the event. This path is relative to the root directory of the service.",
            shortcut: "p",
            type: "string",
          },
          data: {
            usage:
              "String containing data to be passed as an event to your function. Keep in mind that if you pass both --path and --data, the data included in the --path file will overwrite the data you passed with the --data flag.",
            shortcut: "d",
            type: "string",
          },
          stdout: {
      ...
      "rust:stop": {
        usage:
          "Stop all docker container process according to configurations and options",
        lifecycleEvents: ["stop"],
        options: {
          function: {
            usage:
              "The name of the function to stop the docker container. If not given, all the rust function stops",
            shortcut: "f",
            type: "multiple",
          },
        },
    ...
    return {
      ...
      cargoLambda: {
        ...
        profile: _get(
          custom,
          ["cargoLambda", "profile"],
          CargoLambda.profile.release
        ),
        arch:
          this.serverless.service.provider.architecture ||
          CargoLambda.architecture.x86_64,
      },
  ...
  deployArtifactDir(profile) {
    return path.join(this.srcPath, "build", profile);
  }
  ...
  modifyFunctions$(options) {
    ...
    return this.rustFunctions$.pipe(
      map(({ name }) => {
      ...
      mergeMap(({ buildPath, deployPath }) =>
        utils.copyFile(buildPath, deployPath)
      )
    );
  }
  ...
  build$(options) {
    ...
    return this.rustFunctionCount$.pipe(
      tap((total) => {
        if (total === 0) {
          throw this.error(
      ...
      tap(({ result, artifacts }) => {
        if (utils.hasSpawnError(result)) {
          ...
          throw this.error(
            `Rust build encountered an error. Exit code: ${code}. ${error}`
          );
        }
  ...
  rustFunction(funcName) {
    ...
    return {
      ...
      config: {
        containerName:
          _get(func, ["rust", "containerName"]) ||
          `${this.config.service}_${funcName}`,
        port,
        envFile: _get(func, ["rust", "envFile"]) || this.config.local.envFile,
        env: { ...this.config.environment, ...func.environment },
        dockerArgs:
          _get(func, ["rust", "dockerArgs"]) || this.config.local.dockerArgs,
      },
    };
  }
  ...
  get rustFunctions$() {
    ...
    return from(service.getAllFunctions()).pipe(
      filter((funcName) => {
        const func = this.getFunction(funcName);
  ...
  async startContainers() {
    return (
      this.rustContainers$(this.options.function)
        .pipe(
  ...
  showLogs$() {
    const runningContainers$ = this.rustContainers$(this.options.function).pipe(
      filter((container) => container.isRunning)
    );

    const prefixSize$ = runningContainers$.pipe(
      ...
      .pipe(
        ...
        filter((container) => {
          if (Array.isArray(this.containersToStop)) {
            return this.containersToStop.some(
